...
- **Library load** happens once, at page load, before any toggle:
  ...
  `build_lib.sh` builds Ace from source with its module-registry namespace set
...
- `lib/ace/` — upstream Ace 1.43.3, built from source by `./build_lib.sh` with
...
`./build_lib.sh` and re-run it.

An ace bump rebuilds Ace from that tag with `ace-namespace.patch` applied, so
