...
2. `./tools/build_lib.sh` — generates the gitignored `lib/` directory (ace-linters
...
- **Library load** happens once, at page load, before any toggle:
  `ss-fixes.js`'s `init()` calls `loadNewAce()`, which loads the newer library
  from `lib/ace/src-noconflict/` (`ace.js`, `mode-python.js`/`mode-lua.js` for
  the embedded highlighting, `ext-language_tools.js`, `ext-browse_ss.js`,
  `ext-prompt.js`, `ext-statusbar.js`, `ext-settings_menu.js`,
  `keybinding-vim.js`). It lands on `window.__ssAce`, not `window.ace`:
  `tools/build_lib.sh` builds Ace from source with its module-registry namespace set
  to `__ssAce`, so our Ace and SAS Studio's own 1.x Ace have separate
  registries. SAS's global is
  never touched, its stock editor keeps tokenizing against its own build
  (`Mode.js`/`SyntaxColorerAdapter.js`/`SasLexer.js`), and no lazily loaded
  theme/mode/keybinding can end up registered in the wrong library. Only the old
  Ace *stylesheets* are dropped, since both builds use the same `<style>` ids
  and SAS Studio's editor DOM doesn't use them.
- **Activating** installs one-time dispatcher patches on `SAS.Editor` and
  ...
  `caretMoved`.
- **Deactivating** captures each Ace tab's text, cursor, and dirty state,
  destroys the Ace instance, and recreates the original editor via the
  (now-dispatched) `createCodeEditor`, restoring what it captured. SAS Studio's
  stock editor comes back, on the Ace library it always had.
...
at the root. `lib/` is gitignored, generated output: `./tools/build_lib.sh` is the
...
patch from the repo root applied (`tools/ace-namespace.patch` /
`tools/remove-pyright.patch`). `./tools/package.sh` zips
`manifest.json src assets lib` (plus README/LICENSE/CHANGELOG) into
`dist/sas-studio-ext-<version>.zip` for publishing, running `./tools/build_lib.sh`
...
- `src/ace-patches.js` — reproduces at runtime the author's ace-fork source
  changes (github.com/letuanhai/ace) so `lib/ace/` stays upstream code: a theme-aware
...
- `lib/sas-lsp/` — `sas-server.js`, the ~22 MB SAS language server browser
  bundle, built by `./tools/build_lib.sh` from a pinned release tag of
...
activated"). The ~22 MB server bundle is generated by `./tools/build_lib.sh`, not
...
`./tools/build_lib.sh` and re-run it.

An ace bump rebuilds Ace from that tag with `tools/ace-namespace.patch` applied, so
the registry rename comes from Ace's own build rather than a rewrite of the
published files; the smoke test checks that the two Ace libraries stay separate
and that SAS's stock editor still resolves everything it needs from its own
build. Logs are prefixed
